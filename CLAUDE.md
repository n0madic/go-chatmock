...
```
Client Request (OpenAI, Ollama, or Responses API format)
  → proxy/ route handler (openai.go, ollama.go, or responses.go)
    → models.NormalizeModelName()          # alias resolution, effort suffix stripping
    → transform.ChatMessagesToResponsesInput()  # or ConvertOllamaMessages() — skipped for /v1/responses
    → transform.ToolsChatToResponses()     # tool format conversion — skipped for /v1/responses
    ...
    → sse.TranslateChat/Text/Ollama/Responses()  # stream translation back to client format
...
- `/v1/responses` (`proxy/responses.go`) is a near-passthrough: input is already native Responses API format, so no `transform.*` calls are needed. `TranslateResponses` re-emits upstream SSE events verbatim. For non-streaming, output items are collected from `response.output_item.done` events and assembled into a `ResponsesResponse`.
- **Dynamic model registry**: `models.Registry` fetches `GET /backend-api/codex/models?client_version=<ver>` with the same auth headers as the responses endpoint. On first call it blocks to populate; subsequent calls return cached data and refresh in background after 5 min TTL (ETag-conditional GET for bandwidth efficiency). Falls back to `StaticFallback()` if auth fails. All request handlers validate the normalized model name against the registry and return a descriptive 400 error with the list of available models if the model is unrecognised. Validation is skipped when `--debug-model` is set. `info` command displays the live model list.
