...
func ResponsesInputItemsToSDKInput(items []ResponsesInputItem) responses.ResponseNewParamsInputUnion {
	if len(items) == 0 {
...
func responsesInputItemToSDK(item ResponsesInputItem) (responses.ResponseInputItemUnionParam, bool) {
	switch item.Type {
	case "message":
		content := responsesContentToSDK(item.Content, item.Role)
		if len(content) == 0 {
...
func responsesContentToSDK(content []ResponsesContent, role string) responses.ResponseInputMessageContentListParam {
	if len(content) == 0 {
		return nil
	}
...
func ResponsesToolsToSDKTools(tools []ResponsesTool) []responses.ToolUnionParam {
	if len(tools) == 0 {
		return nil
	}
...
func responsesToolToSDK(tool ResponsesTool) (responses.ToolUnionParam, bool) {
	switch tool.Type {
	case "function":
...
func ReasoningToSDK(r *ReasoningParam) shared.ReasoningParam {
	if r == nil {
