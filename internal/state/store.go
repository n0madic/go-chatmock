...
func (s *Store) Get(responseID string) ([]FunctionCall, bool) {
	...
	for _, id := range keys {
		c := e.calls[id]
		out = append(out, FunctionCall{CallID: c.CallID, Name: c.Name, Arguments: c.Arguments})
	}
	return out, true
}
...
func buildCallMap(calls []FunctionCall) map[string]FunctionCall {
	...
	for _, c := range calls {
		...
		callMap[c.CallID] = FunctionCall{CallID: c.CallID, Name: c.Name, Arguments: c.Arguments}
	}
	return callMap
}
